...
public class TaoHoaDon extends JPanel {
     JPanel left,right,luachonsanpham,thongtinsanpham,thongtinsanpham_top,thongtinsanpham_bottom,luachonsanpham_left,right_top,right_bottom,tongsotien,thongtinsanpham_top_matensp,thongtinsanpham_top_chgtsl,thongtinsanpham_bottom_button,thongtinsanpham_bottom_imei,checkbox_imei;
    ...
    if (!event.getValueIsAdjusting()) {  // Chỉ thực thi khi người dùng thực sự chọn
        ...
        sanphamdathemTable.clearSelection();
           ...
           checkbox_imei.removeAll();
            ...
            List<String> imeiList = new ChiTietSanPhamBLL().getImeisBySanPham(Integer.parseInt(sanphamTable.getValueAt(selectedRow, 0).toString()));
            if(imeiList.isEmpty()){
                JOptionPane.showMessageDialog(null, "Sản phẩm bạn chọn đã hết hàng hãy chọn sản phẩm khác hoặc nhập thêm");
                clearOnhap();
            }
      ...
      public void chonsanphamdexoasua(){
            ...
            List<String> imeiList = new ChiTietSanPhamBLL().getImeisBySanPham(Integer.parseInt(tbmsanphamdathemTable.getValueAt(selectedRow, 0).toString()));
            checkBoxes = new ArrayList<>();
            for (String imei : imeiList) {
            JCheckBox checkBox = new JCheckBox(imei);
            checkBox.setAlignmentX(Component.CENTER_ALIGNMENT);
