...
public class DungLuongRomDialog extends JDialog implements MouseListener {
    ...
    RomBLL dlrBUS = new RomBLL();
    ArrayList<RomDTO> list = dlrBUS.getAll();
    ThuocTinhGUI qltt;
//    private final QuyenBLL nhomquyenBus = new QuyenBLL();

    public DungLuongRomDialog(JFrame owner, ThuocTinhGUI qltt, String title, boolean modal) {
...
//    }
    ...
    public void loadDataTable(ArrayList<RomDTO> result) {
//        Collections.sort(result , (r1,r2)->Integer.compare(r1.getDungluongrom(), r2.getDungluongrom()));
        tblModel.setRowCount(0);
        for (RomDTO dlr : result) {
        ...
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                ...
                if (dlrBUS.checkDup(kichthuoc)) {
                    int id = RomDao.getInstance().getAutoIncrement();
                    dlrBUS.add(new RomDTO(id, kichthuoc));
                    loadDataTable(list);
                    ms.setText("");
                } else {
            ...
            if (index != -1) {
                if (e.getSource() == add) {
                    if (Validation.isEmpty(ms.getText())) {
                        ...
                        int kichthuoc = Integer.parseInt(ms.getText());
                        if (dlrBUS.checkDup(kichthuoc)) {
                            dlrBUS.update(new RomDTO(list.get(index).getMadungluongrom(), kichthuoc));
                            loadDataTable(list);
                            ms.setText("");
                        } else {
                            JOptionPane.showMessageDialog(this, "Dung lượng ROM đã tồn tại !");
