...
public class MauSacDialog extends JDialog implements MouseListener {
    ...
    ColorBLL msBUS = new ColorBLL();
    ArrayList<ColorDTO> list = msBUS.getAll();
//    private final QuyenBLL nhomquyenBus = new QuyenBLL();

    public MauSacDialog(JFrame onwer, ThuocTinhGUI qltt, String title, boolean modal) {
...
//    }
    ...
    public void initComponent(ThuocTinhGUI qlttsp) {
        this.qlttsp = qlttsp;
    ...
    public void loadDataTable(ArrayList<ColorDTO> result) {
//        result.sort(Comparator.comparing(ColorDTO::getTenmau));
        tblModel.setRowCount(0);
        for (ColorDTO ncc : result) {
        ...
        if (e.getSource() == add) {
            if (Validation.isEmpty(ms.getText())) {
                ...
                if (msBUS.checkDup(tenmau)) {
                    int id = ColorDao.getInstance().getAutoIncrement();
                    msBUS.add(new ColorDTO(id, tenmau));
                    loadDataTable(list);
                    ms.setText("");
                } else {
            ...
            if (index != -1) {
                if (Validation.isEmpty(ms.getText())) {
                    ...
                    String tenmau = ms.getText();
                    if (msBUS.checkDup(tenmau)) {
                        msBUS.update(new ColorDTO(list.get(index).getMamau(), tenmau));
                        loadDataTable(list);
                        ms.setText("");
                    } else {
                        JOptionPane.showMessageDialog(this, "Màu sắc đã tồn tại !");
